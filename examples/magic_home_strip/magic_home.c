...
homekit_accessory_t *accessories[] = {
    HOMEKIT_ACCESSORY(.id = 1, .category = homekit_accessory_category_lightbulb, .services = (homekit_service_t*[]) {
        HOMEKIT_SERVICE(ACCESSORY_INFORMATION, .characteristics = (homekit_characteristic_t*[]) {
            &name,
            HOMEKIT_CHARACTERISTIC(MANUFACTURER, "KillerMemeStar"),
...
if (current_color.red >= 200 && current_color.green >= 200 && current_color.blue >= 200) {
            // yeee wh
	current_color.white = (current_color.red + current_color.green + current_color.blue) / 3;
        } else {
